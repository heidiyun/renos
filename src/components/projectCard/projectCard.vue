  ...
  <v-card class="c-layout box-shadow round hover"
      ...
      <div class="more-icon">
        <a-dropdown :trigger="['click']"
          ...
          <a-menu slot="overlay">
            ...
            <a-menu-item key="1"
                         @click="renameProjectTitle">이름 변경</a-menu-item>
            <a-menu-item key="3"
                         @click="changeMainImage">이미지 변경</a-menu-item>
            <a-menu-item key="4"
                         @click="pinnedProject">즐겨찾기 {{project.data.pins[$store.getters.user.id] ? '삭제' : '추가'}}</a-menu-item>
          </a-menu>
        </a-dropdown>
      </div>
    </v-img>
    <v-card-title class="pl-3 pr-1 card-title-layout pa-0"
      ...
      <div class="bookmark-outer">
        <v-icon class="ml bookmark"
                :bookmarked="project.data.pins[$store.getters.user.id]"
                size="14"
                @click.stop="pinnedProject">
          {{project.data.pins[$store.getters.user.id] ? 'bookmark':'bookmark_border' }}
        </v-icon>
      </div>
