  ...
  <v-container fluid
    ...
    <v-layout>
        ...
        <div class="content-container"
          ...
          <div class="category px-3 pt-2"
            ...
            <a-input-group compact
              ...
              <a-select @change="onHandleSelectedTagChanged"
                        mode="multiple"
                        placeholder="tag select"
                        color="blue"
                        style="min-width:200px; max-width:fit-content;">
                <a-select-option style="background:green;"
                ...
                <v-menu bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn class="ma-0"
                           style="width:fit-content;"
                           flat
                           v-on="on">{{alignmentKey === 'name' ? '이름' : '업로드 시간'}}</v-btn>
                  </template>
                  <a-list size="small"
                          bordered
                    ...
                    <a-list-item slot="renderItem"
                                 slot-scope="item"
                                 @click="alignmentKey= item === '이름' ? 'name' : 'uploadDate'">{{item}}</a-list-item>
                  </a-list>
                </v-menu>

                <v-btn icon
                       @click="changeAlignmentOrder"
                       class="ma-0">
                  <v-icon small
                          v-if="alignmentKeys.order === 'dsc'">arrow_downward</v-icon>
                  <v-icon small
                          v-else>arrow_upward</v-icon>
                </v-btn>
              </div>
            </div>
          </div>
          <div class="content px-4">
            <div style="width:100%; height:100vh; "
              ...
              <div v-for=" (activity,i) in currentActivities"
                   :key="i"
                   class="mb-4">
                <activity-card style="width:750px; height :200px;"
                               :activity="activity"></activity-card>
              </div>
            </div>
            <v-layout wrap
              ...
              <v-flex style="width:325px; height : 300px;"
                      xl2
                      lg3
                      md4
                      sm6
      ...
      <div class="comment-menu"
           :style="showComment? {display:'inline'} : {display:'none'}">
        <div style="display:flex;">
          <div class="pa-3 subheading font-weight-medium comment-title">{{ showComment === true ? (keyNum == 1 ? project.data.name : $store.getters.selectedFile.data.name) : ''}}</div>
          <v-btn icon
                 flat
        ...
        <a-tabs defaultActiveKey="2"
                @change="keyNum === 1 ? keyNum=2 : keyNum = 1">
          <a-tab-pane tab="프로젝트 댓글"
                      key="1">
            <div style="width:87%; height:calc(100vh - 200px); overflow-y:scroll; justify-content:center; ">
              <div class="px-2 py-1"
          ...
          <a-tab-pane tab="파일 댓글"
                      key="2">
            <div style="width: 87%;height:calc(100vh - 200px); overflow-y:scroll; justify-content:center; ">
              <div class="px-2 py-1"
                   v-for="(comment,i) in currentCommentList"
                   :key="`key2- ${i}`">
                <comment-view :comment="comment"
                              :role="role"
                              :isInput="false"
                              :keyNum="2"></comment-view>
              </div>
              <div class="px-2 py-1">
                <comment-view :comment="null"
                              :isInput="true"
